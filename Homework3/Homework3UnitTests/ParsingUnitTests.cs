...
namespace Homework3UnitTests
{
	...
	public class ParsingUnitTests
	{
		...
		public void TestMethod1()
		{
			...
			emailExamples.Should().HaveCount(1);
			emailExamples[0].IsSpam.Should().BeFalse();
			emailExamples[0].WordsInEmail.Should().HaveCount(c => c > 2);
			emailExamples[0].WordsInEmail["don"].Should().Be(2);

			example["dave"].HamCount.Should().Be(2);
			example["dave"].SpamCount.Should().Be(0);
		}
	}
}
